...
class Embeddings(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        return embeddings
    def incre_forward(self,input_ids,pos_idx):
        ###input_ids: (bs,1,vocab_size)
        ###pos_idx: (bs,1)
        if len(input_ids.size()) == 3:
            y = torch.matmul(input_ids, self.word_embeddings.weight) + self.position_embeddings(pos_idx)
        else:
            ##2
            y = self.word_embeddings(input_ids) + self.position_embeddings(pos_idx)
        y = self.LayerNorm(y)
        y = self.dropout(y)
        return y
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            layer_outputs = layer_module(
                x=hidden_state, attn_mask=attn_mask, head_mask=layer_head_mask, output_attentions=output_attentions
            )
